...
import com.example.lichet.exception.ApiException
import com.example.lichet.exception.AppException
import com.example.lichet.exception.CommunicationException
import com.example.lichet.util.ResourceResolver
        ...
        return this.onErrorResumeNext { throwable ->
            ...
            if (throwable is HttpException && (throwable.code() < 200 || throwable.code() >= 300)) {
                ...
                if (throwable.code() == 404) {
                    return@onErrorResumeNext Single.error(ApiException(throwable, "リソースなし", resourceResolver.getString(
                ...
                try {
                    ...
                    return@onErrorResumeNext Single.error(ApiException(e, "jsonパース失敗", resourceResolver.getString(
                        R.string.toast_something_wrong_happened_on_communication
                    )))
                }

                if (errorResponse?.error?.messages == null) {
                    //エラーオブジェクト、エラーメッセージが取得できない場合は想定外のエラー
                    Single.error(ApiException(throwable, "エラーオブジェクト・エラーメッセージなし", resourceResolver.getString(
                        R.string.toast_something_wrong_happened_on_communication
                    )))
                } else {
                    //エラーメッセージが正常に返却された場合
                ...
                Single.error(CommunicationException(throwable, "接続エラー", resourceResolver.getString(
                    R.string.toast_communication_error_occurred
                )))

            } else if (throwable is UnknownHostException) {

                Single.error(CommunicationException(throwable, "ホスト不明エラー", resourceResolver.getString(
                    R.string.toast_communication_error_occurred
                )))

            } else if (throwable is SocketTimeoutException) {

                Single.error(CommunicationException(throwable, "ソケットタイムアウトエラー", resourceResolver.getString(
                    R.string.toast_communication_error_occurred
                )))
            } else {
